...
หลักการแยกแยะพยัญชนะและสระ (Structural Disambiguation):
1. ระดับกลาง (พยัญชนะหลัก):
   - จะ (ᨧ) vs ตะ (ᨲ) vs คะ (ᨣ) vs ปะ/บะ (ᨷ)
   - สระหน้า: สระแอ (ᩯ), สระเอ (ᩮ), สระโอ (ᩰ), สระใอ (ᩱ), สระไอ (ᩲ)
2. ระดับล่าง (ตัวสะกดห้อย):
   - ว ห้อย (᩠ᩅ) มีลักษณะเป็นวงกลมโค้งตวัดล่าง (เมื่อผสมกับสระแอ ᩯ จะเป็นแม่เกย/สระแอ+ว เช่น ᩯ...᩠ᩅ)
   - ง ห้อย (᩠ᨦ), น ห้อย (᩠ᨶ), ม ห้อย (᩠ᨾ), ด ห้อย (᩠ᨯ), บ ห้อย (᩠ᨷ), ล หาง (ᩖ)
3. ระดับบน (วรรณยุกต์/สระบน):
   - ไม้เหยาะ (ไม้เอก ᩵), ไม้ขอช้าง (ไม้โท ᩶), ไม้ซัด (᩺/᩹), สระอิ (ᩥ), สระอี (ᩦ), ไม้กั๋ง (ᩢ)

ตัวอย่างการประกอบคำสำคัญ:
- "แจ้ว" (ᩯᨧ᩠ᩅ᩵): สระแอ ᩯ + จะ ᨧ + ว ห้อย ᩠ᩅ + ไม้เหยาะ ᩵ = แจ้ว (แปลว่า ว่องไว, คล่องแคล่ว, แจ่มใส หรือเสียงแจ้วๆ)
- "แต่ง" (ᩯᨲ᩠ᨦ᩵): สระแอ ᩯ + ตะ ᨲ + ง ห้อย ᩠ᨦ + ไม้เหยาะ ᩵ = แต่ง (ประดับ, สมรส, จัดการ)
- "แก้ว" (ᩯᨠ᩠᩶ᩅ): สระแอ ᩯ + กะ ᨠ + ไม้ขอช้าง ᩶ + ว ห้อย ᩠ᩅ = แก้ว
- "แพร่" (ᩯᨻᩖ᩵ / ᨻᩯᩕ᩵): สระแอ ᩯ + พะ ᨻ + ล หาง ᩖ + ไม้เหยาะ ᩵ = แพร่ (คนเมืองออกเสียง เมืองแป้)
- "แม่" (ᩯᨾ᩵): สระแอ ᩯ + มะ ᨾ + ไม้เหยาะ ᩵ = แม่
- "ส้าดิบ" (ᩈ᩶ᩣᨯᩥ᩠ᨷ): สะ ᩈ + ไม้ขอช้าง ᩶ + สระอา ᩣ (ส้า) + ดะ ᨯ + สระอิ ᩥ + บ ห้อย ᩠ᨷ (ดิบ)
- "ลาบควาย" (ᩃᩣ᩠ᨷᨤ᩠ᩅᩣ᩠ᨿ): ลาบ + คะ ᨤ + ว ห้อย ᩠ᩅ + สระอา ᩣ + ย ห้อย ᩠ᨿ
- "เวียงกุมกาม" (ᩅ᩠ᨿᨦᨠᩩᨾᨠᩣᨾ): เวียง + กุม + กาม
- "เชียงใหม่" (ᨩ᩠ᨿᨦᩲᩉ᩠ᨾ᩵), "ลำปาง" (ᩃᩣᩴᨻᩣ᩠ᨦ), "กำเมือง" (ᨠᩣᩴᨾᩮᩬᩥᨦ)

ส่งคืนผลลัพธ์เป็น JSON บริสุทธิ์ (Pure JSON เท่านั้น):
{
  "detected_text": "คำแปลหรือคำอ่านภาษาไทยมาตรฐานที่ถูกต้องที่สุด",
