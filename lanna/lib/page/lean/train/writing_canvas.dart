...
class _WritingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showStrokeOrder && orderData != null && orderData.strokes.isNotEmpty) {
      ...
      // 2. วาดกรอบนอกและร่องทางเดินลายเส้น (Track Border & Inner Lane เหมือนรูปที่ 1)
      for (final stroke in orderData.strokes) {
      ...
      // 3. วาดเส้นประคมชัดตรงกลางร่องเส้นเขียน (Dashed Guideline)
      for (final stroke in orderData.strokes) {
        if (stroke.points.isEmpty) continue;
        final strokePath = buildStrokePath(
          stroke.points,
      ...
      // 4. วาดจุดบอกตำแหน่งแต่ละช่วง (Numbered Checkpoints 1, 2, 3...) พร้อมลูกศรสีแดงชี้ทิศทาง
      int globalStepNumber = 1;
      for (final stroke in orderData.strokes) {
        if (stroke.points.isEmpty) continue;
        final isCurrent = stroke.order == completedStrokeCount + 1;

        // คำนวณความยาวสะสมของเส้นเพื่อกระจายจุด Checkpoints ตามสัดส่วน
        ...
        for (final frac in fractions) {
          ...
          // วาดวงกลมสีเขียวสดใสพร้อมตัวเลขสีขาว (Green Badge + Red Arrow)
        ...
        // วาดจุดสิ้นสุดปลายเส้นเป็นวงกลมสีเข้ม
