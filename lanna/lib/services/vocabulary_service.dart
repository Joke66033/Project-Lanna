...
class VocabularyService {
  ...
  Future<VocabularyModel?> getVocabularyById(String id) async {
    final data = await ApiService.get('${ApiConfig.vocabulary}?action=getById&id=$id');
  ...
  Future<List<VocabularyModel>> searchVocabulary(String keyword) async {
    final data = await ApiService.get('${ApiConfig.vocabulary}?action=search&keyword=$keyword');
    if (data == null) return [];
    return (data as List).map((x) => VocabularyModel.fromJson(x)).toList();
  }
  ...
  Future<VocabularyModel> createVocabulary(VocabularyModel item) async {
    final data = await ApiService.post('${ApiConfig.vocabulary}?action=create', item.toJson());
    return VocabularyModel.fromJson(data);
  }

  /// Update vocabulary item
  Future<VocabularyModel> updateVocabulary(String id, Map<String, dynamic> fields) async {
    final data = await ApiService.post('${ApiConfig.vocabulary}?action=update&id=$id', fields);
    return VocabularyModel.fromJson(data);
  }

  /// Delete vocabulary item
  Future<VocabularyModel> deleteVocabulary(String id) async {
    final data = await ApiService.post('${ApiConfig.vocabulary}?action=delete&id=$id', {});
    return VocabularyModel.fromJson(data);
  }
  ...
  Future<List<CategoryVocabModel>> getAllCategories() async {
    final data = await ApiService.get('${ApiConfig.categoryVocab}?action=getAll');
    if (data == null) return [];
  ...
  Future<CategoryVocabModel?> getCategoryById(String id) async {
    final data = await ApiService.get('${ApiConfig.categoryVocab}?action=getById&id=$id');
    if (data == null) return null;
